...
/* GUARANTEE */
.guarantee{padding:56px 24px;background:#fff;border-top:1px solid var(--border)}
.guarantee .container{max-width:960px;margin:0 auto}
.guarantee-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px;margin-top:28px}
.g-item{text-align:center;padding:24px 16px}
.g-icon{font-size:32px;margin-bottom:12px}
.g-item h3{font-family:'Instrument Serif',serif;font-size:18px;margin-bottom:8px}
.g-item p{font-size:14px;color:var(--muted);line-height:1.7}

/* COOKIE BANNER */
.cookie-banner{position:fixed;bottom:0;left:0;right:0;background:#0E1209;border-top:1px solid rgba(242,168,0,.15);padding:18px 24px;z-index:999;gap:16px;align-items:center;flex-wrap:wrap}
.cookie-banner p{flex:1;font-size:13px;color:rgba(255,255,255,.6);min-width:240px}
.cookie-banner a{color:#F2A800}
.cookie-accept{background:#F2A800;color:#0E1209;border:none;padding:10px 22px;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;font-family:'Outfit',sans-serif}
.cookie-reject{background:transparent;color:rgba(255,255,255,.5);border:1px solid rgba(255,255,255,.15);padding:10px 18px;border-radius:8px;font-size:14px;cursor:pointer;font-family:'Outfit',sans-serif}

/* EXIT POPUP */
.exit-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.65);z-index:9999;align-items:center;justify-content:center;padding:20px}
.exit-box{background:#fff;border-radius:14px;padding:36px 32px;max-width:440px;width:100%;text-align:center;position:relative}
.exit-close{position:absolute;top:12px;right:16px;background:none;border:none;font-size:20px;cursor:pointer;color:#888}
.exit-box h3{font-family:'Instrument Serif',serif;font-size:26px;margin-bottom:10px}
.exit-box p{font-size:14px;color:var(--muted);margin-bottom:20px;line-height:1.7}
.exit-cta{display:block;background:var(--gold2);color:var(--ink);padding:14px 28px;border-radius:10px;font-weight:700;font-size:15px;text-decoration:none}
.exit-note{font-size:11px;color:#aaa;margin-top:12px}

/* KOMMUNE GRID */
.kommune-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px;margin:24px 0}

/* DATE BADGE */
.date-badge{display:inline-block;font-family:'Space Mono',monospace;font-size:11px;color:var(--muted);background:var(--mist);padding:4px 10px;border-radius:4px;border:1px solid var(--border);margin-top:8px}

/* TOUCH/MOBILE FIXES */
.btn-next,.btn-submit,.nav-cta{min-height:48px}
...
@media(max-width:380px){.zone-grid{grid-template-columns:1fr}}
</style>`;
...
function form({ zone = '' } = {}) {
  ...
  <div class="form-step" data-step="3">
    ...
    <p class="form-footer">Dine oplysninger deles kun med op til 3 godkendte lokale taglæggere.<br>100% gratis og uforpligtende · Svar inden 2 timer på hverdage kl. 8–17</p>
  </div>

  <div class="form-success">
    <h3>✓ Tak!</h3>
    <p>Dine oplysninger er modtaget. Du vil blive kontaktet af op til 3 lokale taglæggere inden for 2 timer på hverdage kl. 8–17.</p>
  </div>
</form>`;
}
...
const guarantee = () => `<section class="guarantee">
  <div class="container">
    ...
    <div class="guarantee-grid">
      <div class="g-item"><div class="g-icon">⏱</div><h3>Svar inden 2 timer</h3>
        <p>Taglæggerne kontakter dig inden for 2 timer på hverdage kl. 8–17. Gør de ikke det, kontakter vi dig selv og finder en løsning.</p></div>
      <div class="g-item"><div class="g-icon">🔒</div><h3>Dine data er sikre</h3>
        <p>Vi deler dine oplysninger kun med de 3 udvalgte taglæggere i din zone. Vi sælger aldrig data videre.</p></div>
      <div class="g-item"><div class="g-icon">✓</div><h3>100% gratis og uforpligtende</h3>
...
(function(){
  ...
  var uT=m>=2&&m<=7?'🌤 <b>Forårets travle sæson er i gang</b> — book din taglægger nu · Spar op til <b>40.000 kr</b>':m>=8&&m<=9?'🍂 <b>Efterårsvejr nærmer sig</b> — få dit tag tjekket nu · <b>Begrænset kapacitet</b>':'❄️ <b>Planlæg dit tag til foråret</b> — bestil nu og spar <b>op til 40.000 kr</b>';
  ...
  ['adr','pc','by'].forEach(function(id){
    ...
    if(el){
      el.addEventListener('blur',function(){sessionStorage.setItem('form_'+id,el.value)});
      var v=sessionStorage.getItem('form_'+id);if(v)el.value=v;
    }
  });
...
module.exports = { css, head, urgency, topNav, form, ticker, footer, mobileCta, script, cookieBanner, exitIntent, guarantee };
